...
function LandingModules() {
  ...
  const IAMedia = () => (
    <div style={{ height: '100%', position: 'relative', overflow: 'hidden', background: 'radial-gradient(130% 130% at 80% 6%, #2DA15D 0%, #0F5C33 56%, #0A3A22 100%)' }}>
            <img src={(window.IMG && window.IMG.polele) || "../../assets/polele-avatar.svg"} alt="Polélé" width="66" height="66" style={{ position: 'absolute', left: 16, top: 18, borderRadius: '50%', boxShadow: '0 8px 20px -6px rgba(0,0,0,.4)' }} />
  ...
  // Real photo media with light green AgroScan overlay + lazy loading
  ...
  const MODULES = [
    { n: 1, icon: 'map-pinned', accent: 'green', title: 'Mon Champ', desc: 'Cartographiez vos parcelles et calculez automatiquement leur superficie par GPS.', media: <PhotoMedia src={(window.IMG && window.IMG.producer) || "../../assets/photos/producer-smartphone.webp"} alt="Producteur sénégalais cartographiant son champ au smartphone" label="GPS terrain" labelIcon="map-pinned" /> },
    { n: 2, icon: 'satellite', accent: 'green', title: 'Santé des Cultures Pro', desc: 'Analyse satellitaire et surveillance NDVI de la santé de vos cultures, zone par zone.', media: <PhotoMedia src={(window.IMG && window.IMG.field) || "../../assets/photos/field-rice.webp"} alt="Rizière irriguée analysée par satellite" label="NDVI satellite" labelIcon="satellite" /> },
    { n: 3, icon: 'bot', accent: 'green', title: 'IA Agricole — Polélé', desc: 'Polélé, votre conseillère agricole intelligente, disponible 24h/24 en français et en wolof.', media: <IAMedia /> },
    { n: 4, icon: 'cloud-sun-rain', accent: 'blue', title: 'Météo Agricole', desc: 'Prévisions météo locales adaptées à votre exploitation et à vos fenêtres d\u2019intervention.', media: <PhotoMedia src={(window.IMG && window.IMG.sky) || "../../assets/photos/sky-weather.webp"} alt="Ciel de pluie au-dessus d\u2019un champ" tone="blue" label="31°C · pluie" labelIcon="cloud-rain" /> },
    { n: 5, icon: 'camera', accent: 'green', title: 'Diagnostic Maladies', desc: 'Prenez une photo d\u2019une feuille et obtenez un diagnostic instantané, avec plan d\u2019action.', media: <PhotoMedia src={(window.IMG && window.IMG.leaf) || "../../assets/photos/leaf-disease.webp"} alt="Feuilles atteintes par une maladie" label="Analyse photo" labelIcon="scan-line" /> },
  ];
  ...
  return (
    <section className="lp-section lp-container" id="fonctionnalites">
      <div className="lp-center reveal">
        ...
        <p className="lp-lead">La proposition de valeur d'AgroScan Pro : du satellite à l'action, tout ce qu'il faut pour piloter votre exploitation.</p>
      </div>
          ...
          return (
            <article key={m.title} className="lp-mod reveal" data-delay={(i % 3) + 1}>
              ...
              <div className="lp-mod__body">
                ...
                <span className="lp-mod__link" style={{ color: accentStrong }}>Découvrir <Icon n="arrow-right" size={16} color={accentStrong} /></span>
              </div>
            </article>
          );
        })}
        {/* 6th cell: CTA tile */}
        <article className="lp-mod reveal" data-delay="3" style={{ background: 'var(--brand-soft)', border: '1px dashed var(--green-300)', alignItems: 'center', justifyContent: 'center', textAlign: 'center', cursor: 'pointer' }}>
          <div className="lp-mod__body" style={{ alignItems: 'center', justifyContent: 'center' }}>
